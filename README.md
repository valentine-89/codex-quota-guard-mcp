...
Ordinary quota refresh remains caller-driven. During active work, `quota_status` and `job_preflight` expose a `checkAgainBy` deadline: healthy weekly-only work gets up to five minutes; other or near-reserve cases retain 30–60 seconds according to pacing risk. Group related small steps under one admission and avoid paired status/preflight calls. The shared refresh lease, backoff and forecast reserve limit still apply. With no request, no new background reader runs.
...
The MCP also publishes server-wide `instructions`; tool descriptions remain self-contained. The [Codex AGENTS snippet](examples/AGENTS-snippet.md) documents the full host-specific workflow. Global precedence follows [Codex documentation](https://learn.chatgpt.com/docs/agent-configuration/agents-md).
