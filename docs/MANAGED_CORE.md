...
The v0.6 shared core is Codex-bound, not OS-managed. `scripts/install.mjs` provisions private settings and registers `node + dist/connector.js`; it does not create or start a Scheduled Task, service, daemon, login item, launchd agent, or systemd unit.
...
Use `node scripts/uninstall.mjs` to remove registration, or add `--purge` to delete validated v0.6 state.
