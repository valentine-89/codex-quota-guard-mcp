...
4. Only when `canSchedule=true` (blocking reset strictly under 24 hours after grace), pass the returned `automationRequest` unchanged to the host `automation_update` tool. Its same-task one-shot schedule, ownership-bearing name, and fixed `Tiếp tục công việc.` prompt are already complete; do not inspect existing automations, browse scheduling documentation, or rewrite any field. Then call `defer_automation_attach` with the returned `deferId` and created automation ID. A weekly-only `weekly_advisory` caution requires neither defer nor automation.
