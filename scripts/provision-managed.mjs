...
import { mkdirSync, lstatSync, existsSync, writeFileSync, chmodSync } from "node:fs";
...
if (existsSync(path)) {
  ...
  const settings = { revision: 2, installationId: randomUUID(), port, token: randomBytes(32).toString("base64url"),
    nodeExecutable: process.execPath, coreEntrypoint: join(root, "dist", "core.js"), guardConfig };
